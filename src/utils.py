...
class ResNetDecoder(nn.Module):
    ...
    def __init__(self, block, layers, latent_dim, input_height, first_conv=False, maxpool1=False) -> None:
        ...
        self.conv1 = nn.Conv2d(64 * block.expansion, 1,
...
class ResNetEncoderWithFC(nn.Module):
    def __init__(self, block, layers, first_conv=False, maxpool1=False, latent_dim=8):
        super().__init__()
        self.resnet_encoder = ResNetEncoder(
            block, layers, first_conv, maxpool1)
        self.fc_mu = nn.Linear(512, latent_dim)
        self.fc_logvar = nn.Linear(512, latent_dim)

    def forward(self, x):
        x = self.resnet_encoder(x)  # Output is [batch_size, 512]
        mu = self.fc_mu(x)
